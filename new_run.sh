...
for i in {1..1};do
    ...
    if [ -d "$out_dir" ] || [ -d "$contig_dir" ]; then
        echo remove "$out_dir" and "$contig_dir"? Y/N
        read delete
        if [ $delete == "Y" ]; then
            if [ -d "$out_dir" ]; then
    ...
    echo "python3 strain_finder.py $a  --ref=${ref} --narrowing=True --match_l=$match_limit --sam_file=extract.sam --r1_file=${read1} --r2_file=${read2} --round=$i --excluded_IDs=excluded_IDs.txt";
    ...
    if [ -e "$read1" ] && [ -e "$read2" ]; then
        python3 "${DIR}"/strain_finder.py $a $b --ref="${ref}"  --narrowing=True --match_l=${match_limit} --sam_file=extract.sam --r1_file="$read1" --r2_file="$read2" --round="$i" --excluded_IDs="excluded_IDs.txt";
    else
