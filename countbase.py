...
def test_stop_con(ref_file,samfile):
	...
	index_field = strain_finder.index_field
	cigar_field = strain_finder.cigar_field
	read_field = strain_finder.read_field
	spike_range = range(13, 29981)
	ref = ""
	with open(ref_file,"r") as rf:
		for line in rf:
			if line[0]!= ">":
				ref += line.strip()
	read_list = bm.read_sam(samfile)
	initial_matrix_info = bm.matrix_from_readlist(read_list, 1, set({}), True)

	sorted_mutated_read = initial_matrix_info.narrowed_read
	...
	for smr_index in range(len((sorted_mutated_read))):
		...
		if "I" not in sorted_mutated_read[smr_index][cigar_field]:
			...
			insertion = True
		# continue
		print(sorted_mutated_read[smr_index])
		print(ref[smr_read_index:smr_read_index+len(sorted_mutated_read[smr_index][read_field])])
		for i in range(0,len(sorted_mutated_read[smr_index][read_field]),5):
			print(i,sorted_mutated_read[smr_index][3][i:i+5])

		if not insertion:
			stop_con = False
			tmp_misP_set = set(tmp_misP)
			print(tmp_misP,smr_read_index+tmp_misP[0] ,ref[tmp_misP[0]],sorted_mutated_read[smr_index][3][tmp_misP[0]-smr_read_index])
			...
			for mp in tmp_misP:
				...
				for r in protein_loc:
					...
					if mp in r:
					#if r.start <= mp <= r.stop:
						found = True
						# location = int((pos-r.start)/3)
						index = (mp - r.start) % 3
						print(mp-r.start,mp-smr_read_index,index)
						...
						if sorted_mutated_read[smr_index][read_field][mp-smr_read_index] != "-":
							...
							for i in range(mp - index, mp + (3 - index)):
								# if the adjacent spots are also misPs, will use the read's adjecent spot instead of ref
								if i in tmp_misP_set and 0 <= i - smr_read_index < len(
										sorted_mutated_read[smr_index][read_field]):
									mut_codon += sorted_mutated_read[smr_index][read_field][i - smr_read_index]
								else:
									mut_codon += ref[i]

							print(ref_codon,mut_codon,ref_codon[0:index], sorted_mutated_read[smr_index][read_field][mp-smr_read_index],ref_codon[index + 1:])
							if mut_codon == "TAA":
								print("stop codon produced by", sorted_mutated_read[smr_index], tmp_misP)
								stop_con = True

								break
			if sorted_mutated_read[smr_index][cigar_field] == "4S146M" and smr_read_index > 21000:
				exit()
...
def fix_sub_pos(subfile):
	"""
	move index back before soft clipping
	:param subfile:
	:return:
	"""
	readlist = []
	with open(subfile,"r") as sf:
		for line in sf:
			fields = line.strip().split(" ")
			begin_s =  re.match('^([0-9]+)S',fields[st_find.cigar_field])
			if begin_s is not None:
				move = int(begin_s.group(1))
				fields[st_find.index_field] = int(fields[st_find.index_field]) + move
			readlist.append(fields)
...
if __name__ == "__main__":
	...
	if args.read_compare != "no":
		read_compare(input_files[1],input_files[2])
	if args.test_stop_con != "no":
